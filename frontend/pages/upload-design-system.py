"""Upload / connect / describe a design system -> one canonical token JSON + HITL review.
...
from ingestion import hitl_export, manual_wizard
...
def _show(doc):
    st.session_state["tokens_doc"] = doc  # feeds the Dashboard page
    ...
    st.page_link("frontend/pages/dashboard.py", label="View in Dashboard", icon="📊")
...
# Include manual later on.
tab_doc, tab_web = st.tabs(
    ["📄 Document", "🌐 Website"])

with tab_doc:
    st.write("PDF / Word / Excel design spec.")
...
with tab_web:
    st.write("Scrapes a live site and infers tokens.")
